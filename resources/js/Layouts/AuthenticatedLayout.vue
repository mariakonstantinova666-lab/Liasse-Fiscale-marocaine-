...
<template>
    <div class="min-h-screen bg-slate-50">
        <nav class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur">
                ...
                <div class="flex h-16 items-center justify-between">
                    <div class="flex min-w-0 items-center gap-8">
                        <Link :href="route('dashboard')" class="flex shrink-0 items-center gap-3 rounded-lg">
                            ...
                            <span class="hidden sm:block">
                                <span class="block text-sm font-extrabold tracking-tight text-slate-900">LIASSE EXPERT</span>
                        ...
                        <Link
                            ...
                            :class="route().current('dashboard') ? 'bg-blue-50 text-blue-800' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'"
                        >
                            Tableau de bord
                        </Link>
                    </div>
                    ...
                    <div class="hidden items-center sm:flex">
                        <Dropdown align="right" width="48">
                            <template #trigger>
                                <button type="button" class="inline-flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-blue-200 hover:bg-blue-50">
                                    <span class="flex h-8 w-8 items-center justify-center rounded-md bg-blue-100 text-xs font-bold uppercase text-blue-800">
                    ...
                    <button
                        type="button"
                        class="inline-flex h-10 w-10 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 sm:hidden"
            ...
            <div v-show="showingNavigationDropdown" class="border-t border-slate-200 bg-white sm:hidden">
                ...
                <div class="border-t border-slate-100 px-4 py-3">
                    <div class="text-sm font-semibold text-slate-800">{{ $page.props.auth.user.name }}</div>
